...
LLAMA is a graph storage and analysis system that supports mutability,
out-of-memory execution, and streaming built on top of the compressed sparse
row (CSR) representation. Its goal is to perform comparably to immutable
main-memory analysis systems for graphs that fit in memory and to match or
outperform existing out-of-memory analysis systems for graphs that exceed main
memory.
...
Other relevant documents:
  * docs/FILES.txt - short descriptions of all files
  * docs/TODO.txt - a long TODO list, including descriptions of known bugs
...
by defining an appropriate preprocessor directive:
  ...
  * LL_STREAMING - an in-memory sliding window built on top of LLAMA
...
You can further customize streaming by defining:
  * LL_S_SINGLE_SNAPSHOT - use single-snapshot sliding windows
  * LL_S_WEIGHTS_INSTEAD_OF_DUPLICATE_EDGES - maintain edge weights instead of
    storing duplicate edges
...
useful targets are:
  ...
  * benchmark-streaming - uses the streaming / sliding window version of LLAMA
  * benchmark-streaming-weights - uses the streaming version with edge weights
  ...
  * benchmark-ss-streaming - use single-snapshot sliding windows
  * benchmark-ss-streaming-weights - single-snapshot sliding windows + weights
...
benchmark-streaming, benchmark-memory-wd, and benchmark-persistent-wd.
